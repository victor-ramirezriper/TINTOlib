...
def obtener_nombre_dataset(ruta_train):
    ...
    Ejemplo:
        .../EXP_01/EXP_TINTO_TRAIN

    Si existe un archivo de configuración/nombre disponible
    en el futuro, esta función puede sustituirse sin modificar
    el resto del programa.
    """

    # Buscamos primero un archivo CSV cercano a las imágenes.
    ...
    # Si no existe el CSV dentro de TRAIN, intentamos obtener
    # el nombre desde la estructura del experimento.
...
def guardar_registro_csv(ruta_csv, registro):
    ...
    Si el archivo no existe, crea también los encabezados.
    """
...
def seleccionar_experimento():
    ...
    if not metodos_train:
        print(
            f"No se encontraron métodos TRAIN en "
            f"{ruta_lote}."
        )
        return None
    ...
    ruta_train = (
        ruta_lote /
        f"EXP_{metodo_seleccionado}_TRAIN"
    )

    ruta_test = (
        ruta_lote /
        f"EXP_{metodo_seleccionado}_TEST"
    )
...
def entrenar_y_evaluar_cnn(
    ...
    print("\n" + "=" * 60)
    print(
        f"INICIANDO TRANSFER LEARNING CON "
        f"{arquitectura.upper()} (CPU)"
    )
    print("=" * 60)
    ...
    if not ruta_train.exists():
        raise FileNotFoundError(
            f"No existe TRAIN: {ruta_train}"
        )

    if not ruta_test.exists():
        raise FileNotFoundError(
            f"No existe TEST: {ruta_test}"
        )
    ...
    if dataset_train.class_to_idx != dataset_test.class_to_idx:
        ...
        raise ValueError(
            "\nEl mapeo de clases entre TRAIN y TEST "
            "no coincide.\n\n"
    ...
    print(
        f"Imágenes Train: {num_train} | "
        f"Imágenes Test: {num_test}"
    )
    ...
    registro_base = {
        ...
        "tiempo_carga_dataset_s": round(
            tiempo_carga,
            4
        ),
        "tiempo_carga_modelo_s": round(
            tiempo_carga_modelo,
            4
        ),
        "epoca": "",
    ...
    for epoch in range(epochs):
        ...
        acc_epoch = (
            correctos_train.double() /
            len(dataset_train)
        )

        acc_epoch = acc_epoch.item()
    ...
    print(
        f"{'Tiempo entrenamiento':<25} | "
        f"{tiempo_entrenamiento:.2f}s"
    )

    print(
        f"{'Tiempo evaluación':<25} | "
        f"{tiempo_evaluacion:.2f}s"
    )
